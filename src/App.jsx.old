...
const DartsApp = () => {
  ...
  const renderTable = (gamesData, isReadOnly = false) => {
    ...
    return (
      <div className="w-full bg-slate-800 text-white rounded-lg shadow-xl overflow-hidden mb-6 text-sm sm:text-base">
          ...
          <table className="w-full min-w-[800px] border-collapse">
            ...
            <tbody>
              {gamesData.map((game, gIdx) => {
                  ...
                  return (
                  <tr key={game.id} className={`${!isReadOnly && activeGameIdx === gIdx && !isEditing ? 'bg-slate-700' : 'bg-slate-800'} ${isEditing && activeGameIdx === gIdx ? 'bg-slate-700' : ''} ${satOutRowClasses} transition-colors`}>
                    <td className="border border-slate-600 p-2 text-center font-bold text-base bg-slate-700">{game.id}</td>
                    {game.scores.map((score, cIdx) => {
                      ...
                      let cellClasses = `border border-slate-600 p-2 text-center font-mono text-base sm:text-lg h-12 sm:h-14 w-12 sm:w-14 relative transition-all touch-manipulation ${isEditing && !isReadOnly && !game.isSatOut ? 'cursor-pointer hover:bg-slate-700' : ''}`;
                      if (isActive) {
                        cellClasses += ` bg-emerald-200 text-slate-900 ring-2 ring-inset ring-blue-400`;
                      ...
                      return (
                        ...
                        >
                          {isActive ? (
                            <span className="text-slate-900 font-bold text-lg">{currentInput}<span className="animate-pulse">|</span></span>
                          ) : game.isSatOut ? (
                              <span className="text-slate-500 text-xs font-bold">S/O</span>
                          ) : (
                            isTon ? (
                              <div className="flex items-center justify-center w-full h-full">
                                <span className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-red-600 text-white font-bold text-xs">{score}</span>
                              </div>
                            ) : (
                              <span className={isFinishSlash ? 'text-slate-400 font-bold' : 'text-white font-semibold'}>{score}</span>
                            )
                          )}
                        </td>
                      );
                    })}
                    <td className="border border-slate-600 p-2 text-center font-bold text-base bg-slate-900/50">{game.totalScore}</td>
                    <td className="border border-slate-600 p-2 text-center text-slate-300 bg-slate-900/50">{game.totalDarts || '-'}</td>
                    <td className="border border-slate-600 p-2 text-center text-slate-300 bg-slate-900/50">{game.tons}</td>
                    {/* Finish Column */}
                    <td 
                        className={`border border-slate-600 p-2 text-center font-bold bg-slate-900/50 ${isEditing && !isReadOnly ? 'cursor-pointer hover:bg-slate-700 transition-all touch-manipulation' : ''}`}
                        onClick={() => handleFinishCellClick(gIdx)}
                    >
                      <div className="flex justify-center items-center h-full">
                        {game.finish === 'S/O' ? (
                          <UserMinus size={18} className="text-amber-500" title="Sat Out" />
                        ) : game.finish === 'M' ? (
                          <Check size={20} className="text-emerald-400" title="My Finish" />
                        ) : game.finish === 'P' ? (
                          <Check size={20} className="text-orange-400" title="Partner's Finish" />
                        ) : game.finish === '' && game.isComplete ? (
                          <X size={20} className="text-red-500" title="Game Lost" />
                        ) : ''}
                      </div>
                    </td>
                    <td className="border border-slate-600 p-2 text-center text-white font-bold text-base bg-slate-900/50">{game.avg}</td>
                  </tr>
              );})}
            </tbody>
          </table>
          ...
          <div className="w-full grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div className="bg-slate-800 p-4 sm:p-5 rounded-lg border border-slate-700 shadow-xl flex flex-col justify-center">
              ...
              <div className="grid grid-cols-1 gap-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-2xl sm:text-3xl font-bold text-white">{isEditing ? editingTotals.score : currentTotals.score}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-slate-400 text-sm">Total Darts:</span>
                  <span className="text-2xl sm:text-3xl font-bold text-white">{isEditing ? editingTotals.darts : currentTotals.darts}</span>
                </div>
                <div className="flex justify-between items-center pt-3 border-t border-slate-700">
                  <span className="text-emerald-400 font-bold text-sm">1 Dart Avg:</span>
                  <span className="text-3xl sm:text-4xl font-bold text-emerald-400">{isEditing ? editingTotals.avg : currentTotals.avg}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              {isEditing ? (
                ...
                <div className="grid grid-cols-3 gap-2 sm:gap-3">
                  <button 
                      ...
                      className="bg-orange-600 hover:bg-orange-500 disabled:opacity-40 disabled:cursor-not-allowed text-white font-bold py-3 sm:py-4 rounded-lg shadow-lg uppercase tracking-wider text-xs sm:text-sm flex flex-col items-center justify-center gap-1 active:scale-95 transition-all duration-200 touch-manipulation"
                  >
                      <UserMinus size={18} /> <span>SIT OUT</span>
                  </button>
                  <button 
                      onClick={() => handleEndScoreClick()}
                      disabled={activeGameIdx > 2 || games[activeGameIdx]?.isSatOut}
                      className="bg-red-600 hover:bg-red-500 disabled:opacity-40 disabled:cursor-not-allowed text-white font-bold py-3 sm:py-4 rounded-lg shadow-lg uppercase tracking-wider text-xs sm:text-sm flex flex-col items-center justify-center gap-1 active:scale-95 transition-all duration-200 touch-manipulation"
                  >
                      <Hash size={18} /> <span>END SCORE</span>
                  </button>
                  <button 
                      onClick={handleNightComplete}
                      disabled={isEditing}
                      className="bg-purple-600 hover:bg-purple-500 disabled:opacity-40 disabled:cursor-not-allowed text-white font-bold py-3 sm:py-4 rounded-lg shadow-lg uppercase tracking-wider text-xs sm:text-sm flex flex-col items-center justify-center gap-1 active:scale-95 transition-all duration-200 touch-manipulation"
                  >
                      <LogOut size={18} /> <span>NIGHT DONE</span>
                  </button>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-3">
                <button 
                  onClick={handleUndo}
                  className="bg-slate-700 hover:bg-slate-600 text-white font-bold py-3 sm:py-4 rounded-lg shadow-lg uppercase tracking-wider text-sm sm:text-base flex items-center justify-center gap-2 active:scale-95 transition-all duration-200 touch-manipulation"
                >
                  <Undo2 size={20} /> UNDO
                </button>
                <button 
                  ...
                  className="bg-emerald-600 hover:bg-emerald-500 disabled:opacity-40 disabled:cursor-not-allowed text-white font-bold py-3 sm:py-4 rounded-lg shadow-lg uppercase tracking-wider text-sm sm:text-base flex items-center justify-center gap-2 active:scale-95 transition-all duration-200 touch-manipulation"
                >
                    <Hash size={20} /> END SCORE
                </button>
              </div>

              <div className="grid grid-cols-3 gap-2 sm:gap-3 min-h-[200px] sm:min-h-[240px]">
                {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((num) => (
                  <button 
                    key={num} 
                    onClick={() => handleNumPress(num.toString())} 
                    disabled={activeGameIdx > 2 || (isEditing ? editingGameData[activeGameIdx]?.isSatOut : games[activeGameIdx]?.isSatOut)}
                    className="bg-slate-700 hover:bg-slate-600 text-2xl sm:text-3xl font-bold rounded-lg shadow-lg active:scale-95 transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed text-white touch-manipulation"
                  >
                    {num}
                  </button>
                ))}
                <button onClick={handleBackspace} className="bg-slate-700 hover:bg-slate-600 text-xl font-bold rounded-lg shadow-lg flex items-center justify-center active:scale-95 transition-all duration-200 text-white touch-manipulation"><ArrowLeft size={24} /></button>
                <button 
                ...
                <button 
                    onClick={handleEnter}
                    disabled={activeGameIdx > 2 || (isEditing ? editingGameData[activeGameIdx]?.isSatOut : games[activeGameIdx]?.isSatOut)}
                    className="bg-emerald-600 hover:bg-emerald-500 disabled:opacity-40 disabled:cursor-not-allowed text-white font-bold text-base sm:text-lg rounded-lg shadow-lg active:scale-95 transition-all duration-200 touch-manipulation"
                >
                  Enter
                </button>
              </div>
